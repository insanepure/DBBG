...
include_once $_SERVER['DOCUMENT_ROOT'].'/db-bg.de/classes/items/itemmanager.php';
...
class Player
{
  ...
  public function Logout()
  {
	...
	{
		return $this->GetInventory()->GetHasRadar();
	}
	...
	public function HasItem2($id)
	{
		return $this->GetInventory2()->HasItem($id);
	}
	...
	public function AddItems($item, $amount)
	{
		$inventoryItem = $this->GetInventory()->AddItem($item, $amount);
		$items = $this->inventory->Encode();
		$result = $this->database->Update('inventory="'.$items.'"','accounts','id = "'.$this->data['id'].'"',1);
    return $inventoryItem;
	}
	
	public function AddItems2($statsitem, $visualitem, $amount)
	{
		$inventoryItem = $this->GetInventory2()->AddItem($statsitem, $visualitem, $amount);
    return $inventoryItem;
	}
	
	public function RemoveItems($item, $amount)
	{
		$this->GetInventory()->RemoveItems($item, $amount);
		$items = $this->inventory->Encode();
		$result = $this->database->Update('inventory="'.$items.'"','accounts','id = "'.$this->data['id'].'"',1);
	}
	
	public function RemoveItems2($itemid, $amount)
	{
		$this->GetInventory2()->RemoveItem($itemid, $amount);
	}
	
	public function BuyItemFrom($item, $amount, $price, $sellerid)
	{
		$this->BuyItem($item, $amount, $price);
		$result = $this->database->Update('zeni=zeni+'.$price,'accounts','id = "'.$sellerid.'"',1);
	}
	
	public function BuyItemFrom2($statsitem, $visualitem, $amount, $price, $sellerid)
	{
		$this->BuyItem2($statsitem, $visualitem, $amount, $price);
		$result = $this->database->Update('zeni=zeni+'.$price,'accounts','id = "'.$sellerid.'"',1);
	}
	
	public function BuyItem($item, $amount, $price, $arenaprice=0)
	{
		...
		$result = $this->database->Update('inventory="'.$items.'", zeni="'.$zeni.'", arenapoints="'.$arenapoints.'"','accounts','id = "'.$this->data['id'].'"',1);
	}
	
	public function BuyItem2($statsitem, $visualitem, $amount, $price, $arenaprice=0)
	{
		$this->GetInventory2()->AddItem($statsitem, $visualitem, $amount);
	...
	public function SellItem($id, $item, $amount=1)
	{
		...
		$this->SetZeni($zeni);
		$items = $this->inventory->Encode();
		$result = $this->database->Update('inventory="'.$items.'", zeni="'.$zeni.'"','accounts','id = "'.$this->data['id'].'"',1);
		
	}
	
	public function SellItem2($id, $amount=1)
	{
		$item = $this->GetInventory2()->GetItem($id);
		$this->GetInventory2()->RemoveItem($id, $amount);
	...
	public function UnequipItem($slot, $item)
	{
		...
		$items = $this->inventory->Encode();
		$result = $this->database->Update('inventory="'.$items.'", equippedstats="'.$equippedStats.'", travelbonus="'.$travelBonus.'", trainingstats="'.$trainBonus.'"','accounts','id = "'.$this->data['id'].'"',1);
	}
	
	public function UnequipItem2($item)
  {
    $equippedStats = $this->GetEquippedStats();
		$travelBonus = $this->GetTravelBonusInternal();
		$trainBonus = $this->GetTrainBonus();
		...
		$this->GetInventory2()->UnequipItem($item);
	...
	public function EquipItem($id, $item, $slotItem=null)
	{
		$equippedStats = $this->GetEquippedStats();
		$travelBonus = $this->GetTravelBonusInternal();
		$trainBonus = $this->GetTrainBonus();
		if($slotItem != null)
		{
			$this->GetInventory()->UnequipItem($item->GetSlot());
		...
		$this->GetInventory()->EquipItem($id, $item->GetSlot());
		if($item->GetType() == 3)
		{
	...
	public function EquipItem2($item)
	{
		$slotItem = $this->GetInventory2()->GetItemAtSlot($item->GetSlot());
		...
		if($slotItem != null)
		{
			$this->GetInventory2()->UnequipItem($slotItem);
			if($slotItem->GetType() == 3)
			{
				$equippedStats = $this->RemoveEquippedStats($slotItem);
			}
		...
		$this->GetInventory2()->EquipItem($item);
		if($item->GetType() == 3)
		{
			$equippedStats = $this->AddEquippedStats($item);
		}
	...
	public function UseItem2($id, $amount=1)
	{
		$item = $this->GetInventory2()->GetItem($id);
		$this->GetInventory2()->RemoveItem($id, $amount);
  ...
  public function SetStartingPowerup($value)
  {
    ...
    else
    {
		...
		if($this->valid && isset($this->data['inventory']))
		{
			$this->inventory = new Inventory($this->data['inventory'], $this->database, $this);
		}
    $this->inventory2 = new Inventory2($this->database, $this->GetID());
	...
	{
    /*
		$orderKI = '(((mlp / 10)+( mkp / 10) +attack+defense) / 6)';
		$orderStatsFights = '(FLOOR(totalstatsfights/10)*10)';
		$orderLevel = '(level*10)';
		$orderStats = '(stats)';
		$orderKI = $orderKI.'- FLOOR('.$orderStatsFights.'/4) - FLOOR('.$orderLevel.'/4) + FLOOR('.$orderStats.'/4)';
		$order = 'inranking DESC, '.$orderKI;

		$list = new Generallist($database, 'accounts', '*', '', $order, 1, 'desc');
		$entry = $list->GetEntry(0);

		$ki = (($entry['mlp']/10)+($entry['mkp']/10)+$entry['attack']+$entry['defense'])/4;
		$ki = Player::GetSparringKI($ki, $entry['totalstatsfights'], $entry['level'], $entry['stats']);
    */
		$ki = Player::GetSparringKI($player2->GetKI(), $player2->GetTotalStatsFights(), $player2->GetLevel(), $player2->GetStats(), $player2->GetWishes(), $player2->GetStory());

		$playerKI = Player::GetSparringKI($player1->GetKI(), $player1->GetTotalStatsFights(), $player1->GetLevel(), $player1->GetStats(), $player1->GetWishes(), $player1->GetStory());

		$kiDiff = ($ki - $playerKI);
		$statsWin = round($kiDiff * 0.04);
		if($statsWin < 1)
		{
			$statsWin = 1;
		}
	...
	static public function GetSparringKI($ki, $totalStatsFights, $level, $stats, $wishes, $story)
	{
  ...
  public function GetChipLevel()
  {
    ...
    {
      $chipData = $itemManager->GetItem($chipID);
      if($this->HasItem($chipData->GetID()))
        return $level;
      else
      {
        $chipID = $chipData->GetUpgradeID();
        $level++;
      }
    }
    ...
    $this->SetSkillPointLearn(0);
      ...
      $traveledTime = $this->GetActionTime() - round($secondsLeft/60);
        ...
        while($level != $maxLevel)
        {
          ...
          if($this->HasItem($chipData->GetID()))
          {
            $maxLevel = $level+$times;
            if($maxLevel > $this->GetLevel())
              $maxLevel = $this->GetLevel();
            $oldChip = $chipData;
          }
          $chipID = $chipData->GetUpgradeID();
          if($chipID != 0)
            $newChip = $itemManager->GetItem($chipID);
          else
            break;
          $level++;
        }
        ...
        {
          $slot = $oldChip->GetSlot();
          $slotItem = $this->inventory->GetItemAtSlot($slot);
          $isEquipped = $slotItem != null && $slotItem->GetID() == $oldChip->GetID();
          if($isEquipped)
            $this->UnequipItem($slot, $oldChip);

          $this->RemoveItems($oldChip, 1);
          $id = $this->inventory->GetCount();
          $this->AddItems($newChip, 1);

          if($isEquipped)
            $this->EquipItem($id, $newChip);
        }
        else if($oldChip == null)
        {
          $oldChip = $itemManager->GetItem($firstChipID);
          $this->AddItems($oldChip, 1);
        }
    ...
    $zeni = 1000;
		$stats = 10;
		$result = $this->database->Insert('name,userid,race,attacks, zeni, lp, mlp, kp, mkp, attack, defense, fightattacks, place, planet, raceimage, apetail', 
																			'"'.$chara.'","'.$mainID.'","'.$rasse.'","'.$attacks.'","'.$zeni.'","'
																			.($stats*10).'","'.($stats*10).'","'.($stats*10).'","'.($stats*10).'","'.$stats.'","'.$stats.'","'.$fightAttacks.'","'
	...
	public function ContinueStory($levelup, $zeni, $items, $itemManager)
	{
    ...
    if(isset($items) && count($items) > 0)
    {
      foreach($items as &$item)
      {
        ...
        $this->AddItems($itemObject, $itemData[1]);
      }
    }
		$result = $this->database->Update('statspopup="'.$this->GetStatsPopup().'",stats="'.$this->GetStats().'",story="'.$this->GetStory().'",level="'.$this->GetLevel().'",zeni="'.$this->GetZeni().'"','accounts','id = "'.$this->data['id'].'"',1);
	}
  ...
  public function HasWish($value)
  {
	...
	{
		return $this->data['wishcounter'] == 0;
	}
