...
<table width="100%" cellspacing="0">
  ...
  <?php
  $id = 0;
  $list = new Generallist($database, 'actions', 'id', 'race="'.$player->GetRace().'" AND level <= "'.$player->GetLevel().'"', '', 9999, 'ASC');
  $entry = $list->GetEntry($id);
  while($entry != null)
  {
    if($entry['id'] == 48 && $player->GetApeTail() == 0
    || $entry['id'] == 49 && $player->GetApeTail() != 0
    || $entry['id'] == 50 && $player->GetApeControl() == 1)
    {
      ++$id;
      $entry = $list->GetEntry($id);
      continue;
